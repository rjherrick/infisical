...
const initializePassport = async () => {
  passport.use(new GoogleStrategy({
    ...
    try {
      ...
      console.log('passport google A');
      if (!user) {
        console.log('passport google A2');
        user = await new User({
          email,
          authProvider: AuthProvider.GOOGLE,
          authId: profile.id,
        }).save();
        console.log('passport google A3 user: ', user);
      }

      console.log('passport google B');
      ...
      console.log('passport google C');
      req.providerAuthToken = providerAuthToken;
      cb(null, profile);
    } catch (err) {
      console.log('passport google D err: ', err);
      cb(null, false);
    }
  }));
}
